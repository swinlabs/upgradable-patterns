...
async function registerAccount(user_address) {
    ...
    try {
        ...
        // regiser user
        try {
            const gasUsed = await instanceoneLayer.methods.addAccount(user_address).estimateGas();
            console.log(`gas used to add account is ${gasUsed}`);
            const transHash = await instanceoneLayer.methods.addAccount(user_address).send({
                from: administrator,
...
async function getRegisterAccounts() {
    const accounts = await instanceoneLayer.methods.getAccounts().call();
...
async function mintAcg20(_account, _amount) {
    ...
    try {
        var gasUsed = await instanceoneLayer.methods.mint(_account, _amount).estimateGas();
        console.log(`gas used to mint is ${gasUsed}`);
        const transHash = await instanceoneLayer.methods.mint(_account, _amount).send({
            from: administrator,
            gas: Math.floor(gasUsed * 1.5)
        })
...
async function transferAcg20(_from, _to, _amount) {
    try {
        var gasUsed = await instanceoneLayer.methods.transferAcg20(_to, _amount).estimateGas({
            from: _from
        });
        console.log(`gas used to transfer is ${gasUsed}`);
        const transHash = await instanceoneLayer.methods.transferAcg20(_to, _amount).send({
            from: _from,
            gas: Math.floor(gasUsed * 1.5)
        })
...
async function bulkTransferTest(_transNumber) {
    ...
    var totalGasUsed = 0;
    for (var i = 0; i < _transNumber; i++) {
        totalGasUsed = totalGasUsed + await transferAcg20(fromUser, toUser, 10);
        if (i == 99) {
            endTime = new Date();
            console.log(`time to perform is ${(endTime - startTime) / 1000}`);
        }
