...
contract Accounts {
    ...
    modifier onlyAcg20Contract() {
        require(msg.sender == getAcg20ContractAddress(), "Only acg20 Contract is permitted for the operation");
        _;
    }
    ...
    function addAcg20BalanceOf(address _account, uint256 _amount) public onlyAcg20Contract returns (bool) {
        require(isExistingAccount(_account) == true, "account does not exist");
        acg20BalanceOf[_account] = acg20BalanceOf[_account].add(_amount);
        return true;
    }

    function subAcg20BalanceOf(address _account, uint256 _amount) public onlyAcg20Contract returns (bool) {
        require(isExistingAccount(_account) == true, "account does not exist");
    ...
    function addAcg20TotalSupply(uint256 _value) public onlyAcg20Contract returns (bool) {
        acg20TotalSupply = acg20TotalSupply.add(_value);
    }

    function subAcg20TotalSupply(uint256 _value) public onlyAcg20Contract returns (bool) {
    ...
    function addAcg20Allowance(address _from, address _spender, uint256 _amount) public onlyAcg20Contract returns (bool) {
    ...
    function subAcg20Allowance(address _from, address _spender, uint256 _value) public onlyAcg20Contract returns (bool) {
        require(isExistingAccount(_from) == true, "account does not exist");
        require(isExistingAccount(_spender) == true, "account does not exist");
